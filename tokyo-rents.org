...
In this initial phase, we will be collecting rental data. Initially from well-known rental sites in Japan, but later also government and other data which might allow us to better answer our research questions. Our tool of choice is [[https://github.com/scrapy/scrapy][scrapy]], which will give us robust scraping via spiders.
...
#+begin_src web :noweb-ref old-suumo-url :eval no
...
Previous projects also seem to rely on ~BeautifulSoup~. While useful for initial parsing of HTML and CSS based data (and indeed was done for the current project in its early stage), this project will make use of scrapy as a more robust scraper capable of creating data pipelines.
...
First create a spider, ~suumo~, as a class, ~SuumoSpider~, to encapsulate our functions.

*** SuumospiderSpider
:PROPERTIES:
:CREATED:  [2023-08-30 Wed 18:34]
:ID:       b05eaf72-7560-4e4e-8c9e-19bb85d4f832
:END:

Our spider class targeting the 2018 url:
#+begin_src python :noweb-ref suumo-spider :results silent
...
Parser
#+begin_src python :noweb-ref suumo-spider-parser :results silent
...
Implement a crawler process for running in a [[https://www.jitsejan.com/using-scrapy-in-jupyter-notebook][jupyter notebook]]:
...
And make use of the requests library [[https://notebook.community/HrantDavtyan/Data_Scraping/2018/Scrapy][to have a response]] after the initial call for demonstration purposes:
#+begin_src python :results silent
url = "<<old-suumo-url>>"
r = requests.get(url)
response = TextResponse(r.url,body=r.text,encoding="utf8")
#+end_src
...
print(SuumoSpider().get_total_pages(response),"pages were found")
#+end_src

#+RESULTS:
: 705 pages were found

As we can see, the original link still works - albeit with more results than the 2018 project. This can be seen visually on the webpage and is accessible in the =pagination.pagination_set-nav= class.
...
#+begin_src python :exports both :eval no
print(SuumoSpider().get_total_hits(response), "search result hits")
#+end_src

#+RESULTS:
: 310090 search result hits
...
print(SuumoSpider().get_results_per_page(response), "results per page")
#+end_src

#+RESULTS:
: 30 results per page

With a little math we can see that the total number of pages and the results per page don't add up to SUUMO's returned search result hits:

#+name: calculate results
#+begin_src python
SuumoSpider().get_results_per_page(response) * SuumoSpider().get_total_pages(response)
#+end_src
...
After some investigation we can discover that the issue lies with our display options. If we look over to the left side of the menu, we are currently set to "建物ごとに表示 ", or 'Display by building':

[[file:img/tab-ui-buildings.png]]

This causes the returned listings to be displayed by building, with open properties listed below each respective building, as can be seen below:

[[file:img/buildings-display.png]]

Instead of pages being limited to 30 rental listings, they are instead displaying 30 /buildings/ per page, with what every variable number of building specific open properties below each - distorting our calculation of listings.

Instead, by changing the display option to "部屋ごとに表示", 'display by rooms', we can get an accurate number of listings per page:

[[file:img/tab-ui-rooms.png]]

Giving us display of 30 rooms per page:

[[file:img/rooms-display.png]]
...
** suumoscraper
:PROPERTIES:
:CREATED:  [2023-08-30 Wed 18:15]
:ID:       6566af5d-9f46-4408-9a5b-f54194f45075
:END:

#+begin_src python :tangle suumoscraper/__init__.py

#+end_src

*** spiders
:PROPERTIES:
:CREATED:  [2023-08-30 Wed 18:18]
:ID:       c8d10939-0a54-40ec-bd73-0b0686dd40bc
:END:

#+begin_src python :tangle suumoscraper/spiders/__init__.py

#+end_src

#+begin_src python :tangle suumoscraper/spiders/suumospider.py
import scrapy
...
:PROPERTIES:
:CREATED:  [2023-08-30 Wed 18:18]
:ID:       3c1c2f0b-12e3-4de0-a138-d5c34d5d2c8d
:END:
*** middlewares.py
:PROPERTIES:
:CREATED:  [2023-08-30 Wed 18:18]
:ID:       d5fc1c35-b135-473e-9717-472093e19e82
:END:
*** pipelines.py
:PROPERTIES:
:CREATED:  [2023-08-30 Wed 18:18]
:ID:       dccdaefb-9e49-4ed8-8d2f-431dd1270d0a
:END:
*** settings.py
:PROPERTIES:
:CREATED:  [2023-08-30 Wed 18:18]
:ID:       bc446952-f5fc-4653-806f-94d0182f1e2c
:END:
** src
:PROPERTIES:
