...
def fetch_total_pages_count(search_url):
    ...
    try:
        ...
        page_button_text = soup.find("ol", class_="pagination-parts")[-1].text
        last_page_number = int(page_info.split()[-1])
...
We can also see a large number of hits, over 300, 000. Unfortunately, as we'll see later on, this seems to be just a marketing gimmick on part of SUUMO.
