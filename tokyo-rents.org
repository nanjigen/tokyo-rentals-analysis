...
5. Build this into the target collection of rental listings with a generator at =rental_listings=, saving on memory by yielding house elements one by one.
...
#+begin_src python :noweb-ref tests :results silent :eval no
...
#+begin_src sh :session tests :results html :eval yes
pytest tests/test_suumo.py::test_rental_listing_collection
#+end_src
